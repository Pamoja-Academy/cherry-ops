...
export default async function JobDetailPage(props: PageProps<"/jobs/[id]">) {
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <Link href="/jobs" className="text-sm flex items-center gap-1" style={{ color: "#8C8078" }}>
        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs px-2 py-0.5 rounded-full font-medium" style={{ background: "#F3EBE7", color: "#8C8078" }}>
                {job.type}
              </span>
            </div>
            <h2 className="font-display text-2xl font-bold" style={{ color: "#1A1214" }}>{job.title}</h2>
            <div className="flex items-center gap-3 mt-1 text-sm" style={{ color: "#8C8078" }}>
          ...
          <div className="text-right">
            {job.value && <div className="text-2xl font-bold" style={{ color: "#1A1214" }}>{fmt(job.value)}</div>}
            <div className="text-xs" style={{ color: "#8C8078" }}>Job value</div>
          </div>
        </div>
        {job.brief && (
          <p className="mt-4 text-sm leading-relaxed" style={{ color: "#8C8078", borderTop: "1px solid #F3EBE7", paddingTop: 16 }}>
            {job.brief}
          </p>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-xl border p-5" style={{ background: "#fff", borderColor: "#E4D8D1" }}>
          <h3 className="font-display font-bold mb-4" style={{ color: "#1A1214" }}>Tasks</h3>
                  ...
                  <div className="space-y-1.5">
                    {stageTasks.map((t) => (
                      <div key={t.id} className="p-2 rounded-lg border text-xs" style={{ borderColor: "#F3EBE7" }}>
                        <div className="font-medium" style={{ color: "#1A1214" }}>{t.title}</div>
                        {t.assignee && <div style={{ color: "#8C8078" }}>{t.assignee.avatar_initials}</div>}
                      </div>
                    ))}
        ...
        {/* Deliverables */}
        <div className="rounded-xl border p-5" style={{ background: "#fff", borderColor: "#E4D8D1" }}>
          <h3 className="font-display font-bold mb-4" style={{ color: "#1A1214" }}>Deliverables</h3>
          {job.deliverables.length === 0 && <p className="text-sm" style={{ color: "#8C8078" }}>None yet.</p>}
          <div className="space-y-2">
            {job.deliverables.map((d) => (
              <div key={d.id} className="flex items-center justify-between p-3 rounded-lg border" style={{ borderColor: "#F3EBE7" }}>
                <div>
                  <div className="text-sm font-medium" style={{ color: "#1A1214" }}>{d.title}</div>
                  <div className="text-xs" style={{ color: "#8C8078" }}>{d.type} · due {d.due_date}</div>
                </div>
        ...
        {job.allocs.length > 0 && (
          <div className="rounded-xl border p-5" style={{ background: "#fff", borderColor: "#E4D8D1" }}>
            <h3 className="font-display font-bold mb-4" style={{ color: "#1A1214" }}>Studio Allocations</h3>
            <div className="space-y-2">
              {job.allocs.map((a) => (
                <div key={a.id} className="flex items-center justify-between p-3 rounded-lg border" style={{ borderColor: "#F3EBE7" }}>
                  <div className="text-sm font-medium" style={{ color: "#1A1214" }}>{a.resource?.name ?? "—"}</div>
                  <div className="text-sm" style={{ color: "#8C8078" }}>{a.date} · {a.hours}h</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {job.invoices.length > 0 && (
          <div className="rounded-xl border p-5" style={{ background: "#fff", borderColor: "#E4D8D1" }}>
            <h3 className="font-display font-bold mb-4" style={{ color: "#1A1214" }}>Invoices</h3>
            <div className="space-y-2">
