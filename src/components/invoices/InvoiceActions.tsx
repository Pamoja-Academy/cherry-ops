...
export function InvoiceActions({ invoiceId, status }: Props) {
  ...
  const btn =
    "flex items-center gap-1 px-2 py-1 rounded-md text-xs font-semibold border transition-colors hover:bg-wash disabled:opacity-50";
    ...
    <div className="flex items-center gap-1.5">
      {status === "draft" && (
        <button
          ...
          disabled={busy !== null}
          className={btn}
          style={{ borderColor: "#E4D8D1", color: "#1D4ED8" }}
      ...
      {(status === "sent" || status === "overdue") && (
        <>
          <button
            ...
            disabled={busy !== null}
            className={btn}
            style={{ borderColor: "#E4D8D1", color: "#D97706" }}
          ...
          <button
            ...
            disabled={busy !== null}
            className={btn}
            style={{ borderColor: "#E4D8D1", color: "#16A34A" }}
